...
public class WebScreenshotUtils {
    ...
    private static void compressImage(String originImagePath, String compressedImagePath) {
        ...
        try {
            ImgUtil.compress(FileUtil.file(originImagePath), FileUtil.file(compressedImagePath), COMPRESSION_QUALITY);
    ...
    private static void waitForPageLoad(WebDriver webDriver) {
        try {
            ...
            log.error("等待页面加载时出现异常，请重新执行截图", e);
            throw new RuntimeException(e);
        }
    }
    ...
    public static String saveWebPageScreenshot(String webUrl) {
        ...
        try {
            ...
            // 原始文件保存路径
            String imageSavePath = rootPath + File.separator + RandomUtil.randomNumbers(8) + IMAGE_SUFFIX;
            // 访问网页
            ...
            String compressedImagePath = rootPath + File.separator + RandomUtil.randomNumbers(8) + COMPRESS_SUFFIX;
            compressImage(imageSavePath, compressedImagePath);
            log.info("压缩文件保存成功: {}", compressedImagePath);
            // 删除原始图片
